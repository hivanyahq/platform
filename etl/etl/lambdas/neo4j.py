...
def lambda_handler(event, context):
    try:
        ...
        jsonl_output_path = f'/tmp/transformed/{"/".join(object_key.split("/")[2:-1])}.jsonl'
        os.makedirs(os.path.dirname(jsonl_output_path), exist_ok=True)

        airbyte2jsonl_transformer.transform_airbyte2jsonl_format(download_path, jsonl_output_path, mapkey)
        ...
        GRAPH_GENERATOR_MAP = {'confluence': ConfluenceGraphGenerator, 'slack': SlackGraphGenerator, 'jira': JiraGraphGenerator}

        def get_generator(object_key):
            for key in GRAPH_GENERATOR_MAP.keys():
                if key in object_key:
                    logger.info(f"Selected {key} graph generator for object_key: {object_key}")
        ...
        if graph_generator:
            ...
            graph_generator.generate_graph_schema_format_data_files(os.path.dirname(jsonl_output_path), graph_output_directory)
        ...
        return {'statusCode': 200, 'body': json.dumps('File transformed, graph generated, and uploaded successfully')}
