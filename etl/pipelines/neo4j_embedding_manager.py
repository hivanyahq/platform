...
class Neo4jEmbeddingManager:
    ...
    def _generate_embeddings_for_node(self, node, properties, model="text-embedding-ada-002"):
        ...
        embedding = response['data'][0]['embedding']  # Properly access the embedding data
        return embedding
    ...
    def update_embeddings_for_neo4j(self, node_label, node_id):
        ...
        with self.driver.session() as session:
            result = session.run(f"""MATCH (n:{node_label} {{id: $id}}) RETURN n""", id=node_id)
            ...
            """, id=node_id, embedding=combined_embedding)
